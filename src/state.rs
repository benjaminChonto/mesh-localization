...
impl NodeState {
    ...
    pub fn add_distance(&mut self, src_node: [u8; 6], address: [u8; 6], rssi:  i32) {
        ...
        let state_map  = self.neighbours.get_mut(&src_node)
    ...
    pub fn neighbour_matrix(&self) -> Vec<Vec<f32, MAX_SWARM_SIZE>, MAX_SWARM_SIZE> {
        ...
        self.neighbours.keys().for_each(|&node| { let _ = vec.push(node); });
        vec.sort();

        let matrix: Vec<Vec<f32, MAX_SWARM_SIZE>, MAX_SWARM_SIZE> = vec.iter().map(|node| {
            let neighbours = &self.neighbours[node];
            vec.iter()
                .map(|n| *neighbours.get(n).unwrap_or_else(|| {
                    if node == n {
                        return &0.0;
                    }
                    &f32::INFINITY
