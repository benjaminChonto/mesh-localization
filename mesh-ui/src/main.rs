use std::{fmt::format, sync::mpsc::{self, Receiver, Sender, TryRecvError}, thread, time::Duration};
...
fn read_mqtt(tx: Sender<String>) {
    ...
    client.subscribe("mds", QoS::AtLeastOnce)
...
fn render(frame: &mut Frame, logs: &Vec<String>) {
    ...
    let paragraph = Paragraph::new(text)
        .block(Block::new().title("MQTT logs - press q to quit").borders(Borders::ALL));
